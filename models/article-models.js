...
exports.fetchArticleById = (articleId) => {
  return (
    db
      .query(
        `SELECT article_id, title, topic, body, author, created_at, votes, article_img_url, CAST((SELECT COUNT(*) FROM comments WHERE articles.article_id=comments.article_id) AS INTEGER) AS comment_count FROM articles WHERE article_id = $1`,
        [articleId]
      )
      //     `SELECT *
      // FROM articles
